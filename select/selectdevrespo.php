<table class="table table-responsive fontsize12 table-striped table-bordered table-hover table-condensed">
	...
	$resultadd1 = $mysqli->query("SELECT id, empresa, nit, nombres, correo, telfijo, celular, direccion, ciudad FROM responsable WHERE concat_ws(empresa, nit, nombres) LIKE '%".$_POST['selectpickup']."%' AND userid = '".$_POST['userid']."'	ORDER BY empresa ASC LIMIT 10");
	while($rowadd1=mysqli_fetch_array($resultadd1)){
		...
		<tr>
			<td class="colorce0002 fontsize10">									
				<input type="radio" name="responsableid" id="contratistaid" value="<?php echo $rowadd1['id']?>" />
			</td>
			<td class="colorce0002 fontsize10">									
