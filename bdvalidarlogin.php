...
if($rowaccount['id'] != NULL){
	...
	header("Location: http://gpscontrol.cf/fuec/home.php?user_name=".$_GET['user_name']."");
}else{
	if($_GET['user_name'] != NULL AND $_GET['access_token'] != NULL){
		...
		echo '<script>alert("Acceso no autorizado2); window.location="http://gpscontrol.cf/fuec/"</script>';
	}
}
?>