...
if($rowaccount['id'] != NULL){
	...
	header("Location: http://gpscontrol.cf/enlistcontrol/home.php?access_token=".$_GET['access_token']."&user_name=".$_GET['user_name']."");
}else{
	if($_GET['user_name'] != NULL AND $_GET['access_token'] != NULL){
		...
		echo '<script>alert("Acceso no autorizado"); window.location="http://gpscontrol.cf/enlistcontrol/"</script>';
	}
}
?>